...
test('change mark', (t) => {
    let tr = state.tr.addMark({id: 'shortid', ranges: [[2, 2, 2, 2]]}, {key: 'b', val: true});
    let newState = state.apply(tr);
    t.is(newState.doc.sheets['shortid']['cellMetas'][2][2].marks.find(i => i.key === 'b').val, true);

    tr = newState.tr.removeMark({id: 'shortid', ranges: [[2, 2, 2, 2]]}, 'b');
...
test('clear mark', (t) => {
    ...
    tr = newState.tr.clearMark({id: 'shortid', ranges: [[2, 2, 2, 2]]});
    newState = newState.apply(tr);
    t.is(newState.doc.sheets['shortid']['cellMetas'][2][2].marks.length, 0);
});

test('test fmt', (t) => {
    let tr = state.tr.setFmt({id: 'shortid', ranges: [[2, 2, 2, 2]]}, '0,000');
    let newState = state.apply(tr);
    t.is(newState.doc.sheets['shortid']['cellMetas'][2][2].fmt, '0,000');
});

test('test formula', (t) => {
    let tr = state.tr.setFormula({id: 'shortid', ranges: [[2, 2, 2, 2]]}, 'abs(1)');
    let newState = state.apply(tr);
